...
**Where this sits relative to upstream.** Several "Working today" items overlap with fast-moving
`python-roborock` / Home Assistant work (map/georef/wall-zone decode is converging in open PRs) — treat
them as independent, dated corroboration, not unique. The durable, least-duplicated contributions are the
confidence-tagged protocol reference and the REST `/jobs` Hawk *body*-signing finding. Fuller picture: the
README's *Understanding the protocol* section ([README.md](README.md)).
...
- **Room-targeted cleaning** — `clean-rooms <name|id>…` runs a one-time REST job for specific rooms
  (per-job fan/water/route); a complete cycle (undock → clean → dock → charging) is validated live.
...
- **Single-connection daemon** — a long-running helper holds one cloud connection and serves all CLI
  commands over a local socket (no reconnect per command).
...
- **The recurring doorway `501` is environmental, not a gap.** Every run trips a transient cliff-sensor
  fault (501) at a **fixed doorway threshold**; the robot self-clears in seconds and finishes. That sill is
  a **fixed, unchangeable feature of the (rented) apartment**, not a tool limitation — so a "strictly
  fault-free" run is **not a project goal**. The complete clean cycle itself is validated (see "Working
  today").
