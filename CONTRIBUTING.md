...
- **Validate writes inertly.** Anything that triggers robot motion (e.g. `clean-rooms`) must be
  ...
  `PROTOCOL.md` (dry-run fail-safe — post the job disabled so it can't fire even if a delete races the cron).
- **Be gentle with the cloud.** The robot is on a real account; avoid rapid repeated commands
  ...
  `PROTOCOL.md` (single-connection daemon).
- Propose changes via an issue or PR; record significant protocol findings in `DP_DICTIONARY.md` /
  `PROTOCOL.md` and capability results in `CAPABILITIES.md`.

## Scope

`vac.py` controls the device over Roborock's **cloud MQTT** (B01 has no local control channel).
Map/state *reads*, schedule/room-clean *writes*, settings (via the string-key COMMON envelope), and
virtual-wall / no-go-zone SET all work; a couple of settings (`BREAKPOINT_CLEAN`, `MAP_SAVE_SWITCH`)
don't stick. See `CAPABILITIES.md` for the current matrix.
